...
const resendBackup = async (data: RequestData): Promise<void> => {
  ...
  const { error } = await resend.emails.send({
    ...
    react: ServiceRequestTemplateComponent({
      Name: userDetails.Name,
      BusinessName: userDetails.BusinessName || "",
      Email: userDetails.Email,
      Phone: userDetails.Phone,
      MachineId: userDetails.MachineId || "",
      ErrorCode: userDetails.ErrorCode || "",
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    <div>
      <p>
        ${userDetails.Name} just submitted the Service Request form on the Document
      ...
      <p>Machine ID: ${userDetails.MachineId}</p>
      <p>Error Code: ${userDetails.ErrorCode}</p>
      <p>Further Details: ${userDetails.Message}</p>
    </div>`,
    });
